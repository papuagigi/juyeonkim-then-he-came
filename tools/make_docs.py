...
def build_plan():
    ...
    for s in [
        ...
        "**해결** — 바구니를 담으면 ① 겹침 투시(ETF 쌍 겹침률, 여러 ETF에 함께 든 종목, 확인된 실효 비중) ② 구성 투시(투자 지역·자산 유형·위험등급 비율) ③ 발견 규칙 14종(같은 지수 중복, 70% 이상 겹침, 한 종목 집중, 레버리지·커버드콜·합성 구조, 소규모 순자산 등) ④ **생성형 AI 진단서**(근거만으로 쉬운 말, 숫자 사후 대조) ⑤ 이름 해독(사전 " + str(N_TERMS) + "낱말) ⑥ 같은 지수 다른 ETF 비교표를 한 화면에 보여 준다.",
    ...
    for s in [
        "**제작 과정** — 미래에셋 AI 페스티벌(2026.8~9)에서 3인 팀으로 구축한 금융상품 지식베이스(온톨로지 5파일·지식그래프 92만 트리플·한국거래소 구성종목 수집기·해석 사전)를 바탕으로, 본 대회에서는 1인이 '보유 바구니 진단'이라는 다른 용도의 서비스를 새로 설계·구현했다. 화면(모바일 우선 단일 페이지), 투시 계산, 발견 규칙 14종, AI 진단 서버(근거 → HyperCLOVA X → 사후 대조 → 규칙 진단서 대체), 이름 해독 사전은 모두 본 대회를 위해 새로 작성했다.",
...
def build_spec():
    ...
    for s in [
        ...
        "**구현·동작하는 범위** — ① ETF 검색·바구니 담기·금액 입력·예시 바구니 4종 ② 투시 계산(구성 비율 3종, 가중 지표 4종, ETF 쌍 겹침, 공통 종목, 실효 비중) ③ 발견 규칙 14종 ④ 생성형 AI 진단서(HyperCLOVA X + 숫자 사후 대조 + 규칙 진단서 대체) ⑤ 이름 해독(사전 " + str(N_TERMS) + "낱말) ⑥ 같은 지수 다른 ETF 비교표 ⑦ 바구니 주소 공유·브라우저 저장 ⑧ AI 서버 상태 확인.",
    ...
    sub_table(c, ["번호", "기능명", "기능 설명", "관련 화면", "구현 상태"], [
        ...
        ["F6", "발견 규칙", "같은 지수 중복 · 70% 이상 겹침 · 40~70% 겹침 · 공통 종목 · 한 종목 집중 · 지역 몰림 · 전부 주식형 · 레버리지/인버스 · 위험 1등급 비중 · 커버드콜 · 합성복제 · 소규모 순자산 · 고변동성 · 월배당 (+ 이상 없음, ETF 1개 안내)", "④발견", "완료"],
